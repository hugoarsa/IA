(define (domain book-plan)
  ...
  (:functions 
    (numPagesBook ?b - Book)  ;;method that stores how many pages does book ?b have
    (pagesReadMonth ?m - Month) ;;method that stores how many pages have been read on month ?m
  )
  ...
  (:action plan-reading
    ...
    :precondition (and (goal ?b) (not (read ?b)) ;;Book ?b is a book that MUST be read and has not been in order to be put into the plan
            (not (> (+ (numPagesBook ?b) (pagesReadMonth ?m)) 800)) ;;Month ?m must not exceed the monthly page limit of 800 if book ?b is read
    ...
    :effect (and (readMonth ?b ?m) (read ?b) (increase (pagesReadMonth ?m) (numPagesBook ?b)))
  )
